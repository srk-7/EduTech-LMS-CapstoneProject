...
//                                 <p className="text-gray-600 mb-2">ID: {student.studentId}</p> 
...
function StudentList() {
    ...
    return (
        ...
        <div className="p-8 bg-gray-50 min-h-screen">
            ...
            <h1 className="text-4xl font-bold mb-8 text-center text-blue-700">Students</h1>
            {students.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {students.map((student) => (
                        <div key={student.studentId} className="bg-white border border-gray-200 p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105 hover:shadow-xl relative">
                            {/* Delete Button in the top-right corner */}
                            <button
                                onClick={() => handleDeleteClick(student.studentId)}
                                className="absolute top-3 right-3 bg-red-600 text-white p-2 rounded-full hover:bg-red-700 transition duration-200"
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                />
                                <h2 className="text-xl font-semibold mb-1 text-blue-700">{student.name}</h2>
                                <p className="text-gray-500 text-sm"><b>Id:</b> {student.studentId}</p>
                                <p className="text-gray-600 text-sm mt-1"><b>Email:</b> {student.email}</p>
                            </div>
                        </div>
                    ))}
                </div>
